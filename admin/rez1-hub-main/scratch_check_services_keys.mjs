...
async function getServicesKeys() {
  const { data, error } = await supabase.from('services').select('*').limit(1)
  if (error) console.log('services fetch error:', error.message)
  else console.log('services keys:', data[0] ? Object.keys(data[0]) : 'empty table')
}
