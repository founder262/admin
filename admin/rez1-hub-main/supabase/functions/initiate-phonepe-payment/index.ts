...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (clientId) {
      ...
      payloadObj.clientVersion = Number(clientVersion) || 1;
    }

    const base64Payload = btoa(JSON.stringify(payloadObj));
    const secretForSigning = clientSecret || saltKey;
    const stringToSign = base64Payload + "/pg/v1/pay" + secretForSigning;
    ...
    // ── 5. Save PhonePe merchantTransactionId & status to booking ──
    await supabaseAdmin
      .from("bookings")
      .update({
