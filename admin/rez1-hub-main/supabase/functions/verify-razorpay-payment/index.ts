  ...
  try {
    ...
    if (salonOwnerId && salonId) {
      ...
      const { error: alertErr } = await supabaseAdmin.from("owner_booking_alerts").insert({
        ...
        service_summary:
          (updatedBooking?.services || []).map((s: any) => s.name).join(", ") || "Service",
    ...
    const { error: notifErr } = await supabaseAdmin.from("notifications").insert({
      ...
      target_id: updatedBooking?.customer_id,
      title: "✅ Booking Confirmed",
      message: `Your payment of ₹${updatedBooking?.total_amount} has been verified and your booking at ${salonName} is confirmed.`,
      notif_type: "booking",
      is_read: false,
      sent_by_admin: null,
    });
