...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      merchantTransactionId = url.searchParams.get("merchantTransactionId") || url.searchParams.get("transactionId");
      bookingId = url.searchParams.get("bookingId");
    }

    // PhonePe Webhook Validation Ping (Return 200 OK)
    ...
    const secretForSigning = clientSecret || saltKey;
    const stringToSign = `/pg/v1/status/${merchantId}/${merchantTransactionId}` + secretForSigning;
    ...
    if (clientId) {
      ...
      headers["X-CLIENT-VERSION"] = String(clientVersion);
    }
